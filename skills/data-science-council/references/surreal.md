...
Fields: { id, target_id, kind: "document" | "experiment_config", mode: "full" | "lite", verdict, chair_line, consensus[], adjudications[], must_fix[], persona_count, ts, source_text (only if ≤ 8,000 chars, else omit and store a reference) }

Id convention: `verdict:<slug>-<YYYYMMDD>` (slug from target_id or the document title).
