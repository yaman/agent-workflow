...
- Architectural: the human partner reviews and approves the written spec,
  ...
  written-spec approval only permits invoking `story-atdd-workflow`.
...
- **Architectural** — new projects, new subsystems, changes that
  ...
  design, written spec, then the `story-atdd-workflow` skill.
...
9. **Transition to implementation** — invoke the `story-atdd-workflow` skill to turn the spec into stories
...
digraph brainstorming {
    ...
    "Invoke story-atdd-workflow skill" [shape=doublecircle];
    ...
    "User reviews spec?" -> "Invoke story-atdd-workflow skill" [label="approved"];
}
```

**Terminal states are path-bound.** Architectural: the ONLY skill you
invoke after brainstorming is `story-atdd-workflow` — never frontend-design,
...
- Invoke the `story-atdd-workflow` skill to turn the spec into stories. It replaces writing-plans and executing-plans; there are no plan files — the stories and their ACs are the plan.
- Do NOT invoke any other skill. `story-atdd-workflow` is the next step.
